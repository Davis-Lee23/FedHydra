...
class Server(object):
    def __init__(self, args, times):
        ...
        self.unlearn_attack_method = args.unlearn_attack_method
        self.eraser_global_model = None
        self.args = args
    ...
    def aggregate_heads(self):
        ...
        for w, client_model in zip(self.uploaded_weights, self.uploaded_models):
            self.add_heads(w, client_model)

    def add_heads(self, w, client_model):
        for (server_param_name, server_param), (client_param_name, client_param) in zip(
                self.global_model.named_parameters(), client_model.named_parameters()
        ):

            if 'head' in server_param_name:
    ...
    def save_global_model(self):
        ...
        model_path = os.path.join('server_models', self.dataset,
                                  self.algorithm + "_epoch_" + str(self.global_rounds) + ".pt")
        torch.save(self.global_model, model_path)
    ...
    def save_unlearning_model(self):
        print("\nSaving the unlearning model...")
        model_path = os.path.join("server_models", self.dataset)
        if not os.path.exists(model_path):
            os.makedirs(model_path)
    ...
    def server_metrics(self):
        ...
        testloader = DataLoader(testdata, self.batch_size, drop_last=False, shuffle=False)
    ...
    def evaluate(self, acc=None, loss=None):
        # ids, num_samples, tot_correct, tot_auc
        stats = self.test_metrics() # 此处存疑，为什么要让客户端们去评估，应该让全局模型评估才对

        # stats_train:_, num_samples_bad, losses_bad, asr, num_samples_good, losses_good
        # 无投毒的返回：ids, num_samples, losses, ter
    ...
    def select_unlearned_clients(self):
        ...
        self.unlearn_attack_clients = [c for c in self.selected_clients if c.id in ida_]
        # print(self.unlearn_clients, self.remaining_clients)
        print(f"Target clients id: {idx_} \nRemaining clients id: {idr_} \nUnlearn attack clients id: {ida_}." )
        # exit(-1)
    ...
    def build_MIA_attacker(self):
        ...
        import dataset_utils
        testset = dataset_utils.read_all_test_data(self.dataset, self.total_clients)
        testloader = torch.utils.data.DataLoader(testset, self.batch_size, drop_last=False, shuffle=False)
    ...
    def retrain(self):
        ...
        for i in range(self.global_rounds+1):
            ...
            for client in self.remaining_clients:
                ...
                if i == 0:
                    if client in self.unlearn_attack_clients:
