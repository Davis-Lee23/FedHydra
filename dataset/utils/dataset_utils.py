    ...
    if partition == 'pat':
        ...
        for i in range(num_classes):
            ...
            if balance:
                ...
                # 原代码有点问题吧，60000，20client，30-300
                # 修改一下，底数应该至少均值的一半，最高值提高10%
                # num_samples = np.random.randint(max(num_per/10, least_samples/num_classes), num_per, num_selected_clients-1).tolist()
                num_samples = np.random.randint(max(num_per/10, least), num_per*1.5, num_selected_clients-1).tolist()
                ...
                # print(num_per/10)
                # print(num_per)
                # print(least_samples/num_classes)
                print(num_samples)
        ...
        all_indices = [index for indices in dataidx_map.values() for index in indices]
        print(len(all_indices))
        if len(all_indices) != len(set(all_indices)):
            print("Warning: There are duplicate samples in the data assignment.")
...
def split_data(X, y):
    ...
    for i in range(len(y)):
        # 本文作者采用自带的这个train_test_spilt方法，自带的比例是25%，
